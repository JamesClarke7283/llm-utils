    ...
    match cli.command {
        Some(Commands::Estimate {
            ...
            if let Some(model_name) = model_name {
                ...
                table.add_row(row![
                    "Model",
                    "Input Cost (based on input)",
                    "Output Cost (based on output)",
                    "Total Cost"
                ]);
        ...
        Some(Commands::List { file, sort }) => {
            ...
            model_costs.sort_by(|a, b| match sort {
                ...
                crate::commands::SortMetric::FunctionCalling => {
                    b.1.function_calling.cmp(&a.1.function_calling)
                }
                crate::commands::SortMetric::Languages => {
                    b.1.languages.len().cmp(&a.1.languages.len())
                }
            });
